...
#Función para eliminar fichas de los jugadores al jugar en la interfaz
...
def actualizacionFichasJugador(a, b, jugar, i):
  ...
  for j in range(len(jugar)):
    try:#if jugar[j] == (a, b):
      ...
      break
    except ValueError: #elif jugar[j] == (b, a):
      jugar.remove((b, a))
...
#Funcion para el texto que se ingresa en la caja de texto
...
#Estas dos funciones imprimen mensajes en la interfaz
...
def game():
  ...
  while separado[control] != []:
    ...
    if control == 0:
      ...
      r = random.random()
      #Probabilidad de que el bot pase turno 0.1
      if r > 0.1:
        pp = []
        for j in jugar:
          arriba, abajo = j
  ...
  if control == 0:
    imprime_mensaje("Ganaste el juego enhorabuena!!")
  else:
...
def show_game_screen():
  ...
  fichas_3 = random.sample(list(set(fichas) - set(fichas_1) - set(fichas_2)),
                           7)
  ...
  empezar_button = Button(WIDTH / 2 - 25, HEIGHT / 2 - 25, 50, 50, "Play",
                          (255, 255, 255), (0, 128, 0), game)
